...
class FitBackground:
    ...
    def __init__(
        ...
        force : bool
            flag that forces dnu to be set to the `guess`
        guess : float
            `dnu` guess
        clip : bool
            flag that sets the minimum frequency value of the plotted echelle to `clip_value`
        clip_value : float
            lower frequency bound on the echelle plot
        lower_numax : float
            lower bound for numax
        upper_numax : float
            upper bound for numax
        echelle_smooth : bool
            flag that smooths the echelle plot TODO: Currently not supported
        echelle_filter : float
            smoothing filter used when smoothing the echelle given that `echelle_smooth == True` TODO: Currently not supported
        """
        ...
        self.mod_frequency: np.ndarray = None
        self.collapsed_power: np.ndarray = None
    ...
    def update_target(
        ...
        # Unpack target data
        ps_frequency = target_data.ps_frequency
        ps_power = target_data.ps_power
        oversample = target_data.oversample
        ...
        self.resolution = target_data.resolution
        self.nyquist = target_data.nyquist
        # Power spectrum
        self.frequency = fitbg_frequency[mask]
        self.power = fitbg_power[mask]

        # Initial guesses
        self.numax = numax
        self.snr = snr
        self.dnu = dnu
        self.width = WIDTH_SUN * numax/NUMAX_SUN
    ...
    def estimate_harvey_parameters(self, random_power: np.ndarray, b: np.ndarray) -> Tuple:
        ...
        outer_smooth_power = convolve(random_power[~self.mask], boxkernel)
    ...
    def estimate_dnu(
            ...
            deltanu_list: np.ndarray,
            mc_iteration: int
    ) -> Tuple:
        """Estimates delta nu using the auto-correlation function.
        ...
        background_model : np.ndarray
            the fitted background model
        numax_gaussian_parameters : np.ndarray
            the fitted Gaussian parameters used to measure numax
        deltanu_list : np.ndarray
            list containing measurements of deltanu
        mc_iteration : int
        ...
        lag : np.ndarray
            frequency lag of deltanu ACF
        auto : np.ndarray
            auto-correlation of deltanu ACF
        best_lag : float
            frequency lag with highest auto-correlation
        best_auto : float
            highest auto-correlation of deltanu ACF
        lag_peaks : np.ndarray
            frequency lags that correspond to the first `self.n_peaks` of deltanu ACF
        auto_peaks : np.ndarray
            the auto-correlation of the first `self.n_peaks` of deltanu ACF
        zoom_lag : np.ndarray
            zoomed in version of `lag`
        zoom_auto : np.ndarray
            zoomed in version of `auto`
    ...
    def fit_background(self) -> None:
        ...
        dnu_result = self.estimate_dnu(
            region_frequency,
            random_power,
            background_model,
            numax_gaussian_parameters,
            deltanu_list,
            0
        )

        psd, lag, auto, best_lag, best_auto, lag_peaks, auto_peaks, zoom_lag, zoom_auto, dnu_gaussian_parameters = dnu_result
    ...
    def plot_fitbg(self) -> None:
        """Plots result of fit background routine."""

        # Masks
        left_envelope_mask = self.frequency < self.envelope_edges[0]
        right_envelope_mask = self.frequency > self.envelope_edges[1]
        ...
        ax2.plot(self.frequency[left_envelope_mask], self.power[left_envelope_mask], "w-", zorder=0)
        ax2.plot(self.frequency[right_envelope_mask], self.power[right_envelope_mask], "w-", zorder=0)
        ax2.plot(self.frequency[left_envelope_mask], self.smooth_power[left_envelope_mask], "r-", linewidth=0.75, zorder=1)
        ax2.plot(self.frequency[right_envelope_mask], self.smooth_power[right_envelope_mask], "r-", linewidth=0.75, zorder=1)
        ...
        ax3.plot(self.frequency[left_envelope_mask], self.power[left_envelope_mask], "w-", linewidth=0.75, zorder=0)
        ax3.plot(self.frequency[right_envelope_mask], self.power[right_envelope_mask], "w-", linewidth=0.75, zorder=0)
        ax3.plot(self.frequency[left_envelope_mask], self.smooth_power[left_envelope_mask], "r-", linewidth=0.75, zorder=1)
        ax3.plot(self.frequency[right_envelope_mask], self.smooth_power[right_envelope_mask], "r-", linewidth=0.75, zorder=1)
        for r in range(self.num_laws):
            ax3.plot(
                self.frequency,
                harvey(
        ...
        ax6.set_ylim(
            [
                min(self.auto) - 0.05*(max(self.auto) - min(self.auto)),
                max(self.auto) + 0.1*(max(self.auto) - min(self.auto))
            ]
        )
        ...
        ax9.set_ylim(
            [
                min(self.collapsed_power) - 0.025*(max(self.collapsed_power) - min(self.collapsed_power)),
                max(self.collapsed_power) + 0.05*(max(self.collapsed_power) - min(self.collapsed_power))
            ]
        )
    ...
    def get_ridges(self, mc_iteration: int, start: float = 0.0) -> None:
        """Calculate echelle properties.
        ...
        if nox > 2 and noy > 5:
            ...
            for ii in range(len(gridx)):
                for jj in range(len(gridy)):
                    use = np.where(
                        (modx >= startx)
                        & (modx < startx + self.dnu/nox)
                        & (self.frequency >= starty)
                        & (self.frequency < starty + self.dnu)
                    )[0]
                    if len(use) == 0:
                        arr[ii, jj] = np.nan
                    else:
            ...
            extent = [
                min(gridx) - self.dnu/nox/2.0, 2*max(gridx) + self.dnu/nox/2.0,
                min(gridy) - self.dnu/2.0, max(gridy) + self.dnu/2.0
            ]
    ...
    def write_bgfit(self, results: list) -> None:
        """Writes result of fitbg to output file.

        Parameters
        ----------
        results : list
            the results of the fit background routine
        """
